...
Route::group(["namespace" => 'Admin', 'middleware' => ['auth.admin', 'auth.screenlock', 'plan.null'], 'prefix' => 'admin'], function () {
    ...
    Route::get('getNhif/{gross}',['as' => 'admin.getNhif', 'uses' => 'PayrollsController@calculateNhif']);
    Route::get('getPayee/{gross}',['as' => 'admin.getPayee', 'uses' => 'PayrollsController@calculatePayee']);
    Route::get('getNssf/{gross}',['as' => 'admin.getNssf', 'uses' => 'PayrollsController@calculateNssf']);
    Route::get('multiple','PayrollsController@multiple');


    Route::any('payrollSummary','PayrollsController@payrollSummary')->name('payrollSummary');
    Route::any('approvePayroll','PayrollsController@approvePayroll')->name('approvePayroll');
    Route::any('makePayments','PayrollsController@makePayments')->name('makePayments');
    Route::get('downloadSummary/{m}/{y}','PayrollsController@downloadSummary')->name('downloadSummary');
    Route::get('downloadPaymentSchedule/{m}/{y}','ReportController@downloadPaymentSchedule')->name('downloadPaymentSchedule');
    Route::post('approveSelectedPayrolls','PayrollsController@approveSelectedPayrolls')->name('approveSelectedPayrolls');
    Route::get('paySelectedPayrolls/{id}','PayrollsController@paySelectedPayrolls')->name('paySelectedPayrolls');

    // reports

    Route::any('p9a','ReportController@p9a')->name('p9a');
    // Route::any('employeesReport','ReportController@employeesReport')->name('employeesReport');
    Route::any('statutoryDeductions','ReportController@statutoryDeductions')->name('statutoryDeductions');
    Route::any('paymentShedule','ReportController@paymentShedule')->name('paymentShedule');
    Route::any('employeeReport','ReportController@employeeReport')->name('employeeReport');
    Route::any('payrollVariations','ReportController@payrollVariations')->name('payrollVariations');
    Route::get('downloadVariations/{m}/{y}','ReportController@downloadVariations')->name('downloadVariations');
